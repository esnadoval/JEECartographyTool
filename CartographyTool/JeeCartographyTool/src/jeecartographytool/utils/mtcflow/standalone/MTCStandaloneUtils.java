...
public class MTCStandaloneUtils {
	...
	public static void initializeRegistry() {
		...
		MTCFlow.TechnologyManagerRegistry.INSTANCE.put("mtc", new MTCTechnologyManager());
	...
	public static MTCChoreographer startMTC(IProject project, String bundleName, String mtcName, Transformation transformation, List<Model> startModels, List<String> tagList, HashMap<String, Object> variables) {
		...
		try {
			...
			if (console == null) {
				console = new EclipseConsole(findConsole("Migration Tool"));
			}
			try {
				// show the console
				...
				view.display(findConsole("Migration Tool"));
			} catch (Exception e) {
				//e.printStackTrace();
			}
			...
			//mtcChoreographer.setExecutorService(new SingleThreadExecutorService());
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
		}
